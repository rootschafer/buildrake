---
name: run-buildrake
description: Build, run, and smoke-test buildrake. Use when asked to run buildrake, try the CLI, verify a change works in the real binary, or exercise its prompts/flags end-to-end.
---

buildrake is a single-binary CLI that finds Cargo projects under a directory
...
`.claude/skills/run-buildrake/smoke.sh`. All paths are relative to the repo
root.
...
```bash
cargo build          # binary at target/debug/buildrake
```
...
```bash
.claude/skills/run-buildrake/smoke.sh
...
```bash
./target/debug/buildrake --path "$FIXTURE" --dry-run --show-size
...
```bash
printf 'y\n' | ./target/debug/buildrake --path "$FIXTURE"
```

Always set `BUILDRAKE_CONFIG` to a nonexistent path first (the driver does
this) — see Gotchas.
...
README usage block, `docs/src/cli-reference.md`, `docs/man/buildrake.1` —
...
- **Your real `~/.config/buildrake/config.toml` silently changes runs.**
  Any manual invocation should `export BUILDRAKE_CONFIG=/nonexistent` (the
...
- **A bare `target/` directory is not enough for the scan.** buildrake only
